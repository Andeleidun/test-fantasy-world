...
const sections = [
  ...
  ['cosmology', 'Magic & belief', 'The Otherworld, magic and belief', 'Everyday practice, spiritual encounters, attributed traditions and their limits.'],
  ...
  ['maps', 'Atlas', 'The public atlas guides', 'Thirteen geographic subjects describe landscapes, habitats and journeys. Revised map artwork is not yet available.'],
  ['reference', 'References', 'Cultural histories and references', 'Detailed accounts, public beliefs and the collection index.']
];
...
await emit('index', layout('World guide', 'home', `${intro('World guide', 'Erde, Dverghamar & Merenval', 'Explore the landscapes, living communities, languages, magic and beliefs of Erde, Dverghamar, Merenval and its companion. Choose a world or browse a subject below.')}${worlds}<section class="section-block" aria-labelledby="explore-title"><div class="section-heading"><h2 id="explore-title">Browse by subject</h2></div><div class="entry-grid">${['elves','stellar-system-reference','magic','otherworld','korvar'].map(id => card(catalog.find(p => p.id === id))).join('')}</div></section><aside class="note"><strong>Reading the setting</strong><p>These accounts distinguish observation, natural-history inference, tradition and belief. Spiritual encounters and Otherworld stories retain their uncertainties.</p><a href="about.html">About this guide <span aria-hidden="true">→</span></a></aside>`));
...
for (const [id, name, title, description] of sections) {
  ...
  if (id === 'maps') body += `<aside class="note"><strong>Geographic reading notes</strong><p>The current edition contains text guides. Earlier detailed map artwork awaits a separate public revision.</p></aside>`;
...
await emit('about', layout('About this guide', 'about', `${intro('World guide', 'About this guide', 'The public cultural edition of Erde, Dverghamar, Merenval and their connected setting.')}<article class="prose"><h2 id="scope">What you’ll find here</h2><p>Explore landscapes, peoples, histories, languages, ordinary magic and beliefs. Natural history reflects what careful observers can infer; spiritual accounts describe practitioner experience; Otherworld accounts preserve reports and traditions without presenting them as a complete explanation.</p><p>Some names, local histories and biological details remain undescribed. Belief, inference and observation retain their separate status.</p><h2 id="sources">Sources and the public edition</h2><p>This edition follows the current <a href="https://drive.google.com/drive/folders/126BrWsD3GZ-Qg2oGTjxX3lN_Tp642AXt">Public — Cultural Knowledge collection</a>, synchronized on 18 September 2026. Browse the <a href="README.html">complete collection index</a> or the <a href="reference.html">cultural references</a>. Earlier technical drafts and unrevised maps are outside this edition.</p><p>All original project IP remains the author’s. Public documentation grants no license or right to reuse the work.</p><h2 id="downloads">Language data and indexes</h2><ul><li><a href="data/korvar-lexicon.tsv">Korvar dictionary (TSV)</a></li><li><a href="data/regional-dictionary.tsv">Regional dictionary (TSV)</a></li><li><a href="data/korvar-examples.tsv">Translated examples (TSV)</a></li><li><a href="lexicon.json">Lexicon (JSON)</a></li><li><a href="maps.json">Atlas subjects (JSON)</a></li><li><a href="catalog.json">Document catalog (JSON)</a></li><li><a href="PUBLIC-INDEX.txt">Source collection index (text)</a></li></ul><p>The collection contains ${catalog.length - 1} subject articles, its collection index, ${lexicon.length} lexical records and thirteen atlas subjects. Map artwork is awaiting revision; the atlas can be read as text.</p><h2 id="accessibility">Reading preferences</h2><p>The guide follows your device’s light or dark appearance until you choose a theme. That choice is remembered when browser storage is available. Printed pages use a light background.</p><p>Articles, atlas notes, downloads and the complete dictionary remain available without JavaScript. Search and dictionary filtering use JavaScript. Keyboard navigation includes a skip link, visible focus and scrollable reference tables. The Stavmark writing chart has an accompanying text explanation and full-size link.</p></article>`));
