...
The ordinary hand is linear, written left to right in horizontal lines from top to bottom. Each sign has an upright stem and an orientation hook at its upper left; short angled branches distinguish the signs. Carving, ink, printing and raised tactile marking can preserve the same identities. An orientation hook keeps a rotated loose label from silently becoming ordinary text. The careful teaching forms deliberately use pairs of branches: a single missing branch produces a malformed base letter, although real wear can still destroy information. This is a design aid, not a demonstrated error-correcting writing system or an in-world digital encoding theory.
...
Decimal counting is the implemented standard. This is a cultural design choice, independent of the unselected digit count. It supports a familiar shared arithmetic without assuming that biological six-limbedness forces base six. The written decimal figures in this edition use the familiar 0–9 reader convention; their native straight-stroke counterparts are supplied on the chart. They have a baseline marker that separates figures from letters. Their straight segments are practical marks, not evidence of electronic displays.
...
Parentheses in technical writing group arithmetic expressions; spoken calculations say **op del** and **luk del**, open/close part, around a grouped expression. Grouped mathematical language is a conventional technical register, not an additional interpretation of every ordinary command to open a chamber. Very large magnitudes can use powers of ten expressed as repeated multiplication or written indexed powers; everyday readers are not assumed to use scientific notation fluently. The supplied cardinal generator covers zero through 999,999,999; larger numbers are expressible by the same grouping conventions without a claim that the program handles them.

## Time, units and industrial records

**Tik** is a clock-counted short time unit, **takt** a scheduled activity/rest cycle, **orvtid** an orbital period, and **tid** duration generally. These do not import terrestrial solar days. A date record specifies its local/shared epoch and counted units; the language is usable without this document choosing the world's political epoch or calendar reform. A clock can be mechanical. People can say earlier, later, frequently and after a rest without observing a sunrise.

**Mer**, **vek**, and **kap** are conventional length, mass and capacity units. Their calibrations must accompany quantitative conversion into Earth SI. This edition supplies language for units and tolerances without inventing a dwarven body-based meter or fixing unselected economic standards. **Trykk**, **spenn**, **straum**, **mot** and **lut** identify measured pressure, potential difference, current, resistance and useful thermal conditions; careful records state the actual temperature scale or operational parameter rather than treating 'warmth' as a numerical temperature definition.

A technical record contains sample/asset identifier, location, time, property, measurement method, value and unit, conditions, responsible observer, and optional vis/tal/ren evidence marker. This is a recommended record format, not a required grammar for ordinary conversation. A practical label might read:

> Trov 14. Durfinrak. Tör. Gred af stalmenum: ne vitet.

> Sample 14. Dark fine-grained rock. Dry. Grade for support stone: not known.

These are labeled fragments. A full assertion is **Da gred af stalmenum sere ne vitet**, the support-stone grade is not known. The label's uncertainty is informative; the term durfinrak supplies no hidden certification. A real engineering specification would add the relevant measurements, not merely replace uncertainty with a favorable adjective.
...
Accessible communication can use large-print or raised Stavmark, written readback and agreed visual/tactile instructions. Dwarven communities may also have full signed languages with their own histories. This edition does not misrepresent a set of work gestures or fingerspelling as a complete signed language, and it does not infer a signed grammar from four arms. The present plan's implementation covers the specified spoken family and its writing system.

## Two connected reading passages
...
The corpus gives each sentence's construction. The passages illustrate continuous reference and ordinary use; they are not geological test protocols or declarations of new named settlements.
...
3. **Act and remember.** Build present, past and perfect forms. 'We built the nursery' = **Vir karnte da nerkel.** 'We have built the nursery' = **Vir have karnet da nerkel.** The perfect relates the event to the present; it is not merely an ornate past ending.
...
6. **Produce a connected text.** Describe a home, an ordinary need, one material observation and a request. Check each content word in the dictionary, each preposition's case, the finite verb position and the source of any uncertainty claim. The corpus supplies models and contrasts rather than one mandatory story.

This course is a complete entry sequence into the reference edition. Further fluency requires using and enlarging the language in stories and conversations, just as a grammar and dictionary do not by themselves constitute every speaker's lifelong experience.
