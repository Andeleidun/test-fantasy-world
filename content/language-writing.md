...
**Stavmark**, 'letter-mark', is the edition's working native script. Its 24 base signs cover eight vowel qualities and sixteen consonants, including one sign for ng /ŋ/. Phonemic vowel length repeats the vowel sign. The chart accompanying this handbook supplies every base sign and the two regional extensions sh and dh. The glyphs were designed for this language.

The ordinary hand is linear, written left to right in horizontal lines from top to bottom. Each sign has an upright stem and an orientation hook at its upper left; short angled branches distinguish the signs. Carving, ink, printing and raised tactile marking can preserve the same identities. An orientation hook keeps a rotated loose label from silently becoming ordinary text. The teaching forms use pairs of branches. A missing branch leaves a visibly malformed letter, helping readers notice damage. Heavy wear can still make a sign unreadable. The design has not been tested as an error-correcting system.
...
Decimal counting is the implemented standard. This cultural choice is independent of limb and digit counts. The written decimal figures in this edition use the familiar 0–9 reader convention; their native straight-stroke counterparts are supplied on the chart. They have a baseline marker that separates figures from letters. Their straight segments suit carved and written records.
...
Parentheses in technical writing group arithmetic expressions; spoken calculations say **op del** and **luk del**, open/close part, around a grouped expression. In mathematical contexts, these expressions conventionally mark grouping. Very large magnitudes can use powers of ten expressed as repeated multiplication or written indexed powers; everyday readers are not assumed to use scientific notation fluently. The supplied cardinal generator covers zero through 999,999,999; larger numbers are expressible by the same grouping conventions without a claim that the program handles them.
...
Accessible communication can use large-print or raised Stavmark, written readback and agreed visual/tactile instructions. Dwarven communities may also have full signed languages with their own histories. Work gestures and fingerspelling can complement speech, but a full signed grammar remains undescribed. This edition covers the spoken family and its writing system.
...
The corpus explains each sentence’s construction. These passages illustrate ordinary use; their places are examples.
...
For further practice, use the grammar and dictionary to write conversations and stories.
